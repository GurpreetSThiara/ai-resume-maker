...
export function EducationSection({ data, onUpdate }: EducationSectionProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {(educationSection?.items || []).map((education, index) => (
          <Card key={index} className="relative">
            {editingEducationId === index.toString() && !isMobile ? (
      ...
      {/* Mobile edit sheet */}
      {isMobile && editingEducationId !== null && (
        <RecordFormSheet
          open
          onOpenChange={(o) => { if (!o) { setEditingEducationId(null); setEditData({ institution: "", degree: "", startDate: "", endDate: "", location: "", highlights: [] }) } }}
