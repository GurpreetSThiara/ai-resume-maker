...
export const CreativeATSResume: React.FC<ResumeProps> = ({
  pdfRef,
  font,
  theme,
  resumeData,
  ...
  return (
    <div ref={pdfRef} className={`bg-gray-50 min-h-screen p-6 ${font.className}`} style={{ fontFamily: font.name }}>
      <div className="max-w-3xl mx-auto bg-white shadow border rounded-lg p-8">
        {/* Header */}
        <div ref={personalInfoRef} className="pb-4 border-b border-gray-300">
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight" contentEditable suppressContentEditableWarning onBlur={handleNameChange}>
            {resumeData.name}
          </h1>
        ...
        {/* Custom Fields */}
        <div ref={customFieldsRef} className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 mt-4 text-xs">
