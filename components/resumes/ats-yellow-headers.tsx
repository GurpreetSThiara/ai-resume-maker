...
export const ATS_YELLOW_HEADERS: React.FC<ATSYellowHeadersProps> = ({
  resumeData,
  setResumeData,
  className = "",
}) => {
  ...
  const handleSkillsChange = (sectionId: string, value: string) => {
    ...
    setResumeData((prev: ResumeData) => {
      ...
      const section = updated.sections.find((s) => s.id === sectionId)
      if (!section) return prev
      if (section.type === SECTION_TYPES.SKILLS || section.type === SECTION_TYPES.LANGUAGES) {
        section.items = [value]
      }
      return updated
    })
