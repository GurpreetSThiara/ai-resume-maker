...
export function ReviewSection({ data, template }: ReviewSectionProps) {
  const completionStats = {
    personalInfo: !!(data.name && data.email),
    education:
      data.sections.find((s) => s.title === "Education")?.content &&
      Object.keys(data.sections.find((s) => s.title === "Education")?.content || {}).length > 0,
    experience:
      data.sections.find((s) => s.title === "Professional Experience")?.content &&
      Object.keys(data.sections.find((s) => s.title === "Professional Experience")?.content || {}).length > 0,
    skills:
      data.sections.find((s) => s.title === "Other")?.content &&
      Object.keys(data.sections.find((s) => s.title === "Other")?.content || {}).length > 0,
  ...
  const resumeMetadata = {
    ...
    totalFields:
      ...
      data.sections.reduce((acc, section) => acc + Object.keys(section.content).length, 0),
  ...
  function estimateWordCount(resumeData: ResumeData): number {
    ...
    wordCount += countWords(resumeData.name)
    wordCount += countWords(resumeData.email)
    wordCount += countWords(resumeData.phone)
    wordCount += countWords(resumeData.location)
    wordCount += countWords(resumeData.linkedin)
